import IDrawingLayerToolState, {
  ExportGeoJSON,
} from "./../../../tools/layers/drawing/model/types/tool/IDrawingLayerToolState";
...
class GeovistoMap extends MapObject implements IMap {
  ...
  public redraw(
    ...
    let mapContainer: HTMLElement | null = document.getElementById(
      this.getState().getId()
    );
  ...
  public initialize(initProps: IMapInitProps): this {
    ...
    if (!toolsManager.isEmpty()) {
      ...
      for (let i = 0; i < tools.length; i++) {
        ...
        tools[i].initialize({
          config: initProps.configManager.getToolConfig(tools[i].getId()),
          map: this,
          geojson: initProps.geojson,
        });
    ...
    const toolsConfigs:
      | IMapToolConfig[]
      | undefined = initProps.configManager.getToolsConfigs();
    if (toolsConfigs != undefined) {
      ...
      for (let i = 0; i < toolsConfigs.length; i++) {
        ...
        if (toolConfig.type) {
          const toolTemplates: IMapTool[] = <IMapTool[]>(
            toolTemplatesManager.getByType(toolConfig.type)
          );
  ...
  public export(): Record<string, unknown> {
    return this.getState()
      .getMapConfig()
      .export(this.getState().serialize(this.getDefaults()));
  }
  ...
  protected createMapContainer(): HTMLElement | null {
    const mapContainer: HTMLElement | null = document.getElementById(
      this.getState().getId()
    );
    if (mapContainer) {
      const mapElement = mapContainer.appendChild(
  ...
  protected createMap(): L.Map {
    ...
    const map: L.Map = L.map(this.getContainerId(), {
      ...state.getInitialMapStructure(),
    }).setView(state.getInitialMapCenter(), state.getInitialZoom());
  ...
  public updateCurrentData(
    ...
    this.getState()
      ...
      .scheduleEvent(
        ...
        }, // update current data in map state
        undefined
      );
  }
}
