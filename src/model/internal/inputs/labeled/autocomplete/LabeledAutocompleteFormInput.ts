...
class LabeledAutocompleteFormInput
  ...
  protected createForm(): HTMLDivElement {
    ...
    labelDiv.innerHTML = (<ILabeledAutocompleteFormInputProps>(
      this.getProps()
    )).label;
  ...
  protected createListeners(): void {
    if (this.input) {
      ...
      this.input.onchange = (<ILabeledAutocompleteFormInputProps>(
        this.getProps()
      )).onChangeAction;
      ...
      this.input.addEventListener(
        ...
        (e: KeyboardEvent) => {
          if (this.completionListDiv != undefined) {
            ...
            if (
              this.selectedCompletionItem != -1 &&
              completionItems[this.selectedCompletionItem] != undefined
            ) {
              completionItems[this.selectedCompletionItem].classList.remove(
            ...
            if (e.key === "ArrowDown" || e.keyCode === 40) {
              ...
              this.selectedCompletionItem =
                (this.selectedCompletionItem + 1) % completionItems.length;
              e.preventDefault();
              ...
              this.selectedCompletionItem =
                (this.selectedCompletionItem + completionItems.length - 1) %
              ...
              // key Enter
              this.input.value =
                completionItems[this.selectedCompletionItem].textContent ?? "";
            ...
            if (completionItems[this.selectedCompletionItem] != undefined) {
              completionItems[this.selectedCompletionItem].classList.add(
                COMPONENT_COMPLETION_ACTIVE_ITEM_CLASS
              );
            }
  ...
  protected createMenu(): void {
    ...
    const createCompletionItem = (label: string, className: string) => {
      ...
      completionItemDiv.addEventListener("click", () => {
        this.input.value = completionItemDiv.textContent
          ? completionItemDiv.textContent
          : "";
